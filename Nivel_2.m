function varargout = Nivel_2(varargin)
% NIVEL_2 MATLAB code for Nivel_2.fig
%      NIVEL_2, by itself, creates a new NIVEL_2 or raises the existing
%      singleton*.
%
%      H = NIVEL_2 returns the handle to a new NIVEL_2 or the handle to
%      the existing singleton*.
%
%      NIVEL_2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in NIVEL_2.M with the given input arguments.
%
%      NIVEL_2('Property','Value',...) creates a new NIVEL_2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Nivel_2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Nivel_2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Nivel_2
 
% Last Modified by GUIDE v2.5 24-Jul-2019 09:02:38
 
% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Nivel_2_OpeningFcn, ...
                   'gui_OutputFcn',  @Nivel_2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Nivel_2 is made visible.
function Nivel_2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Nivel_2 (see VARARGIN)
 
% Choose default command line output for Nivel_2
...
set(handles.Nivel_2,'visible', 'on');        %Ponemos visible el boton de iniciar nivel 2
...
function varargout = Nivel_2_OutputFcn(hObject, eventdata, handles) 
...
% --- Executes on button press in Nivel_2.
...
%---------------------------------------------------------------------------
%-- 2. VISUALIZACIÓN DEL JUEGO AL PRESIONAR EL BOTÓN DE INICIAR NIVEL 2 ---
%---------------------------------------------------------------------------
 
% Volvemos visible o invisible las componentes del axis -----------------------------
set(handles.Nivel_2,'visible', 'off');       %Ponemos invisible el boton de iniciar nivel 2
...
cont=330;           % Contador independiente utilizado para que despues de cierto tiempo aparezca un hongo bueno o un hongo malo
x=0;                % variable que me indicara si la siguiente figura es un hongo bueno o un hongo malo
...
nivel1=imread('./Imagenes/nivel2.jpg');   % Leemos la imagen
...
while(1) % Ciclo infinito donde se encuentra gran parte del desarrollo del juego
    ...
    if(x==1 || x==3) % Cuando salga un hongo malo se pinta cuadrado rojo y se reconoceran los objetos verdes
        ...
        % camara reconozca
         for object = 1:length(objetos)
            ...
            if (xCent>Xcuadrado && xCent<Xcuadrado+WidthCuadrado && yCent>Ycuadrado && yCent<Ycuadrado+HighCuadrado)
                disparo=1;
                Xcuadrado=round(3*rand)*WidthCuadrado; % Genera números enteros aleatorios entre 0 y 7 y lo multiplica por el numero de pixeles de columnas
                Ycuadrado=round(2*rand)*HighCuadrado; % Genera números enteros aleatorios entre 0 y 7 y lo multiplica por el numero de pixeles de filas
            end
         end
    end
    %----------------------------------------------------------------------
    ...
    if disparo==1
        display('DISPARÉ');
        disparo=0;
        if(x==1 || x==3) % Si el hongo malo estaba en la pista
            cont=330;
            p=0;
        end
    end
    ...
    % Aparecerá un hongo bueno o uno malo aleatoriamente-----------------------
    ...
    if(cont>=360)
        x=randi(3); % La funcion randi entrega a x 1,2 o 3 de forma aleatoria
        cont=0;
    end
 
    if(x==1 || x==3) % Cuando x sea igual a 1 o 3 aparecerá el hongo malo
        p=p+30; 
    elseif(x==2) % Cuando x sea igual a 2 aparecerá el hongo bueno
        k=k+30; 
    end
 
    if(p==300) % cuando p sea igual a 300 (el hongo malo solapa al pacman) el hongo malo debe desaparecer
        x=0;
        p=0;
        cont=330;
    elseif(k>330) % cuando k sea mayor a 330 (el hongo bueno llega al limite de la pantalla) el hongo bueno debe desaparecer
        k=0;
        x=0;
    end
    ...
    %-- 12. PIERDES VIDAS -------------------------------------------------
    % --------------------------------------------------------------------- 
    if(p==270) % Cuando el hongo malo toca al pacman, se pierde una vida
    ...
    % ---------------------------------------------------------------------    
    %-- 14. PASAR AL SIGUIENTE NIVEL --------------------------------------
    % --------------------------------------------------------------------- 
 
    if(puntaje==5)          % Cuando acumulas 6 puntos pasas al siguiente nivel
        clear sound;        % Paramos el sonido del juego
        close(Nivel_2)      % Cerramos el archivo actual
        Nivel_3             % Abrimos el archivo del siguiente nivel
    end
